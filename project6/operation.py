...
#copied and modified yours as mine were wrong
def lookup(id, stk):
...
def resolve(e, stk = []):
    ...
    if isinstance(e, Expr):
        return resolve_expr(e, stk)
    if isinstance(e, Type):
        return resolve_type(e, stk)
    assert False
    
#resolves expressions
def resolve_expr(e, stk):
    if type(e) is BoolExpr:
        # No ids here.
        return e
    ...
    if type(e) is TupleExpr:
        # TODO: Need to resolve all subexpression or maybe not
        return e

    assert False

#resolves types
def resolve_type(t, stk):
    #only one instance of these two so just return it
    ...
    if type(t) is UniType:
        new = stk + [{p.id:p for p in t.parms}]
        resolve(t.type, new)
        return t
